...
public class LinkController {
	...
	public ResponseEntity inserirLink(@RequestBody @Valid LinkModel link, BindingResult resultado) {
	...
	public ResponseEntity atualizarLink(@RequestBody @Valid LinkModel link, BindingResult resultado) {
		ResponseEntity resposta = null;
		if (resultado.hasErrors()) {
			resposta = new ResponseEntity(
					new Mensagem("erro", "os dados sobre o link  não foram informados corretamente"),
	...
	public ResponseEntity removerLink(@RequestBody @Valid LinkModel link, BindingResult resultado) {
		ResponseEntity resposta = null;
		if (resultado.hasErrors()) {
			resposta = new ResponseEntity(
					new Mensagem("erro", "os dados sobre o link  não foram informados corretamente"),
	...
	public ResponseEntity<Object> inserirLinks(@RequestBody @Valid Iterable<LinkModel> links, BindingResult resultado) {
		...
		try {
			if (resultado.hasErrors()) {
				...
				Iterable<LinkModel> l = coletorService.salvarLinks(links);
				if (l != null) {
	...
	public ResponseEntity<Object> encontrarLink(@PathVariable(value = "url") String url) {
		ResponseEntity<Object> resposta = null;
		List<LinkModel> links = coletorService.encontrarLinkUrl(url);
		if (!links.isEmpty()) {
	...
	public ResponseEntity<Object> listarEmOrdemAlfabetica() {
		ResponseEntity<Object> resposta = null;
		List<LinkModel> links = coletorService.listarEmOrdemAlfabetica();
		if (!links.isEmpty()) {
			resposta = new ResponseEntity<Object>(links, HttpStatus.OK);
		} else {
			resposta = new ResponseEntity<Object>(HttpStatus.NO_CONTENT);
	...
	public ResponseEntity<Object> getPagina(@PathVariable(value = "pagina") int pag) {
		ResponseEntity<Object> resposta = null;
		PaginaModel result = coletorService.obterPagina(pag);
		if (result != null) {
	...
	public ResponseEntity listar(@PathVariable(value = "host") String host) {
		ResponseEntity<Object> resposta = null;
		HostModel hosts = coletorService.obterPorHost(host);
		if (hosts != null) {
	...
	public ResponseEntity<Object> encontrarLinkPorIntervaloDeData(@PathVariable(value = "date1") Date date1,
		...
		ResponseEntity<Object> resposta = null;
		List<LinkModel> resultado = coletorService.pesquisarLinkPorIntervaloDeDataUltimaColeta(date1, date2);
